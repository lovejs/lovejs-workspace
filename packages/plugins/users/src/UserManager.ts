...
export class UserManager {
    ...
    constructor(repository, encoder, tokenProvider, socialManager, emitter, cache, options) {
     ...
     */
    async signIn(user) {
        return this.tokenProvider.getToken(user.get("id"));
    }
    ...
    async updateEmailValidate(token) {
        ...
        if (await this.repository.getEmailExists(newEmail)) {
    ...
    async social(currentUser, service, token) {
        const profile = await this.socialManager.getProfile(service, token);
